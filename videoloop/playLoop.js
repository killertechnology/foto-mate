...
function playImages(photoString){
	...
	delay(200).then(() => {//
		...
		try {
			...
			var _cmd = "sudo fbi --noverbose '" + photoString[0] + "' " + _img0ResizeAttr  + " -a -T 1 -t 5 " + photoString[0];
		...
		delay(4000).then(() => {//
			...
			try {
				...
				shell.exec("sudo fbi --noverbose '" + photoString[1] + "' " + _img1ResizeAttr  + " -a -T 1 -t 5 " + photoString[1]);// + " 2>/dev/null");// --noverbose 
		    } catch (ex) {
		        console.log('Error playing video:', ex);
		        //shell.cd('sh reset.sh');
		    } finally {
			...
			delay(4000).then(() => {//
				...
				try {
					...
					shell.exec("sudo fbi  --noverbose '" + photoString[2] + "' " + _img2ResizeAttr  + " -a -T 1 -t 5 " + photoString[2]);// + " 2>/dev/null");// --noverbose 
				...
				delay(4000).then(() => {//
					...
					try {
						...
						shell.exec("sudo fbi  --noverbose '" + photoString[3] + "' " + _img3ResizeAttr  + " -a -T 1 -t 5 " + photoString[3]);// + " 2>/dev/null");// --noverbose 
			        	//	console.log('Video too long. Ignored')
