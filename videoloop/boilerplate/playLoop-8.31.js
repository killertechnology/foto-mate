...
*/		
		...
		try {
			_thisVideo = theseVideos[0].split('|');
	       	if (_thisVideo[1] > 20) { _thisVidStartPos = "00:00:20";} else{ _thisVidStartPos="00:00:00" }
			_cmd = 'sudo omxplayer -l ' + _thisVidStartPos + ' -o local  ' + _thisVidScreenSize + _thisVideo[0]
...
function playImages(photoString){
	...
	delay(200).then(() => {//
		...
		try {
			...
			shell.exec("sudo fbi '" + photoString[0] + "' " + _img0ResizeAttr  + " -a -T 1 -t 5 -cachemem 0 " + photoString[0]);// 2>/dev/null");// --noverbose        
	    } catch (ex) {
	        console.log('Error playing video:', ex);
	        //shell.cd('sh reset.sh');
	    } finally {
		...
		delay(4000).then(() => {//
			...
			delay(4000).then(() => {//
				...
				try {
					...
					shell.exec("sudo fbi '" + photoString[2] + "' " + _img2ResizeAttr  + " -a -T 1 -t 5 -cachemem 0 " + photoString[2]);// + " 2>/dev/null");// --noverbose 
				...
				delay(4000).then(() => {//
					...
					try {
						...
						shell.exec("sudo fbi '" + photoString[3] + "' " + _img3ResizeAttr  + " -a -T 1 -t 5 -cachemem 0 " + photoString[3]);// + " 2>/dev/null");// --noverbose 
			        	//	console.log('Video too long. Ignored')
